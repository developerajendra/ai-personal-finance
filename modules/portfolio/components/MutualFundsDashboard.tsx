...
import { ZerodhaMutualFund } from "@/core/services/zerodhaService";
...
export function MutualFundsDashboard() {
  ...
  const totalValue = mutualFunds.reduce(
    (sum, mf) => sum + mf.last_price * mf.quantity,
    0
  );
  const totalPnl = mutualFunds.reduce((sum, mf) => sum + mf.pnl, 0);
              ...
              {mutualFunds.length === 0 ? (
                <tr>
                  <td colSpan={9} className="text-center py-8 text-gray-500">
                    No mutual funds found. Connect your Zerodha account to view holdings.
                  </td>
                </tr>
              ) : (
                mutualFunds.map((mf) => {
                  ...
                  return (
                    <tr key={mf.folio} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap font-semibold">{mf.fund_name}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{mf.tradingsymbol}</td>
                      <td className="px-6 py-4 whitespace-nowrap font-mono text-sm">{mf.folio}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {mf.quantity.toLocaleString("en-IN", {
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        ₹{mf.average_price.toLocaleString("en-IN", {
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        ₹{mf.last_price.toLocaleString("en-IN", {
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td
                        className={`px-6 py-4 whitespace-nowrap ${mf.pnl >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
                        {mf.pnl >= 0 ? "+" : ""}
                        ₹{mf.pnl.toLocaleString("en-IN", {
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td
                        className={`px-6 py-4 whitespace-nowrap ${
                          mf.pnl_percentage >= 0 ? "text-green-600" : "text-red-600"
                        }`}
                      >
                        {mf.pnl_percentage >= 0 ? "+" : ""}
                        {mf.pnl_percentage.toFixed(2)}%
                      </td>
                    </tr>
                  );
                })
